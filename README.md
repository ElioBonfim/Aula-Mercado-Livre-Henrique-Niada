...
A **calculadora de margem** (`public/calculo-margem.js` e `public/calculadora.html`) é um porte do motor da calculadora do **Henrique Niada** (`HN-devs-mentoria/calculadora-margem-hn`), de React+Vite para um arquivo só que roda no navegador e no Node. As fórmulas e as tabelas de comissão são as dele, sem alteração; os 73 testes vieram junto. Confirme com o autor antes de redistribuir fora desta aula.
